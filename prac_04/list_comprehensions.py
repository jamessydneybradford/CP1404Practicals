...
numbers = [int(item) for item in almost_numbers]
print(numbers)

# TODO: list comprehension to create a list of only the numbers that are
# greater than 9 from the numbers (not strings) you just created
numbers_over_nine = [int(item) for item in numbers if item > 9]
...
names_over_eleven = [' '.join(name.split()[1]) for name in full_names if len(name) > 11]
print(names_over_eleven)